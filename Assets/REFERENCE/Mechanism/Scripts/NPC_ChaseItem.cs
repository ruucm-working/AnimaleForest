...
/// </summary>
public class NPC_ChaseItem : MonoBehaviour {
	...
	{	
		...
		m_Temtation = GameObject.FindWithTag ("Temtation");	
		Debug.Log ("m_Temtation : "+m_Temtation);

//		m_PlayerAnimator = m_Temtation.GetComponent<Animator>();
//		Debug.Log ("m_PlayerAnimator : "+m_PlayerAnimator);
		Debug.Log ("ShouldChaseItem : "+ShouldChaseItem());
	...
	bool ShouldChaseItem()
	{
		float distanceToTemtation = Vector3.Distance(m_Temtation.transform.position, transform.position);		


		Debug.Log ("distanceToTemtation : "+distanceToTemtation);


		if(distanceToTemtation < m_AttackDistance)
		{
...
//			}			
	...
	void OnAnimatorMove()
	{
		if(CheatRoot)
		{
			...
			{
				m_LookAtPosition.Set(m_Temtation.transform.position.x, transform.position.y, m_Temtation.transform.position.z); // Kill Y.
	...
	void SpawnBullet() 
	{
		...
		Vector3 direction = m_Temtation.transform.position - BulletSpawnPoint.position;
		direction.y = 0;
